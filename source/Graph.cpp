...
void Graph::printNodesStatus()
{
        ...
        for (int v = 0; v < nNodes; ++v)
        {
                ...
                cout << v << ":\tparent=" << (nodes[v].parent == nNodes ? -1 : nodes[v].parent) << "\ts=" << nodes[v].s << "\te=" << nodes[v].post << endl;
...
void Graph::computeLabels()
{
        ...
        while (Q.size() > 0)
        {
                ...
                for (const auto &node : Q)
                {
                        ...
                        unsigned int worker_id = hash(parent) % parent_workers.size();
                        parent_workers[worker_id].addTask([this, parent, node, &sem, worker_id]() -> void {
                                if (nodes[node].adj.size() == 0) {
                                        nodes[node].s = nodes[node].post;
                                        
                                } else {
                                        for(const auto child: nodes[node].adj) {
                                                if(nodes[node].s > nodes[child].post) nodes[node].s = nodes[child].post;
                                        }
                                } 
                                if (nodes[parent].adj.size() == ++nodes[parent].lab_visited_count)
                                        C.push(parent);
                                sem.signal();
                        });
